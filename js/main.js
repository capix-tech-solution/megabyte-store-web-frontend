...
stepUpdate = (steps_content, step_counter, progressbar, action) => {
	...
	action == 'next' ? progressbar[step_counter - 1].classList.replace('w-0', 'w-full') : null;
	action == 'prev' ? progressbar[step_counter].classList.replace('w-full', 'w-0') : null;

	action == 'next' ? step_number[step_counter].classList.add('active') : null;
	action == 'prev' ? step_number[step_counter + 1].classList.remove('active') : null;
};
...
element && element.forEach((element) => {
	element.addEventListener('click', (e) => {
		...
		setTimeout(() => {
			const parent = document.querySelector(target.getAttribute('data-target'));
